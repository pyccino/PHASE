function [lonlatIN_AOI_STC2D, dates_full, t_full, final_signal_orig, final_signal_out, final_std_out] = ...
    STmodel_STC2D(displIN_AOI, PSidIN_AOI, t_dateIN, t_relIN, x_grid, y_grid, use_inclined_means, figsDir, minMonths, ...
    gS_input_path, gS_output_path, gS_job_path, detectedOS, dtCov_STC2D, dsCov_STC2D, utmZone, xyIN_AOI, xyAOI, step_t, ...
    detrend_method, poly_degree, tCovModel_STC2D, sCovModel_STC2D, markerSize_STC2D)

% STmodel_STC2D Performs spatio-temporal stochastic modelling for displacement data
%
% Inputs:
...
%   x_grid                - matrix with gridded x-coordinates for estimation
%   y_grid                - matrix with gridded y-coordinates for estimation
%   use_inclined_means    - flag for horizontal (true = means only) vs. tilted planes (false = full a + b*x + c*y)
...
%   xyAOI                 - coordinates of the AOI buonding box
...
%
% Output:
%   lonlatIN_AOI_STC2D    - lon/lat coordinates of estimated grid points
...
disp('--- 2D Stochastic-based modelling started... ---');
...
while iter < 2
    ...
    if strcmp(detrend_method, 'residualAtm')
        ...
        if ~use_inclined_means
            ...
            spatialMeans_grid_temp = B_grid + (Sx_grid .* X_grid) + (Sy_grid .* Y_grid);
        end
    ...
    elseif strcmp(detrend_method, 'cleanObs')
        ...
        x_full_n = x_full_centered;
        y_full_n = y_full_centered;
        
        A_full_grid = ones(length(t_full_n), 1);

        if poly_degree >= 1
            A_full_grid = [A_full_grid, x_full_n, y_full_n, t_full_n];
        end
        
        if poly_degree >= 2
             A_full_grid = [A_full_grid, x_full_n.^2, y_full_n.^2, t_full_n.^2, ...
                            x_full_n.*y_full_n, x_full_n.*t_full_n, y_full_n.*t_full_n];
        end
        
        if poly_degree >= 3
             A_full_grid = [A_full_grid, x_full_n.^3, y_full_n.^3, t_full_n.^3, ...
                      x_full_n.^2.*y_full_n, x_full_n.^2.*t_full_n, ...
                      y_full_n.^2.*x_full_n, y_full_n.^2.*t_full_n, ...
                      t_full_n.^2.*x_full_n, t_full_n.^2.*y_full_n, ...
                      x_full_n.*y_full_n.*t_full_n];
        end
    ...
    disp('Computing global empirical covariance...');
        ...
        if isnan(dsCov_STC2D)
            dsCov = prctile(sqrt((xyIN_AOI(:,1)).^2 + (xyIN_AOI(:,2)).^2), 25);
        else
            dsCov = dsCov_STC2D;
        end 
        max_dist = sqrt(range(xyIN_AOI(:,1))^2 + range(xyIN_AOI(:,2))^2);
        edges = 0:dsCov:ceil(max_dist / dsCov) * dsCov;
        ...
        parfor i = 1:n_epochs
            ...
            if sum(valid) > 1
                ...
                switch method_flag
                    ...
                    [~, s_sq1, n_p1] = f2DEmpCovEst_accum(sp_val, coords_val, edges);
                ...
                [local_acc, s_sq, n_p] = f2DEmpCovEst_accum(sp_input, coords_val, edges);
    ...
    xy_obs_all = [grid_t_flt_col, grid_x_flt_col, grid_y_flt_col];
    [C_obs_obs_m, ~, C_vv_m] = compute_C_obs_obs_nugget_2D(xy_obs_all, ...
...
xy_obs = [grid_t_flt_col, grid_x_flt_col, grid_y_flt_col];
valid_obs = ~isnan(xy_obs(:, 1)) & ~isnan(xy_obs(:, 2)) & ~isnan(xy_obs(:, 3));
...
% Define spatial grid 
dx_grid = abs(x_grid(1,1)-x_grid(1,2));
dy_grid = abs(y_grid(1,1)-y_grid(2,1));
x_full = (min(x_grid(:)):dx_grid:max(x_grid(:)))'; 
y_full = (min(y_grid(:)):dy_grid:max(y_grid(:)))'; 

[grid_x_full, grid_y_full, grid_t_full] = meshgrid(x_full, y_full, t_full); 

% Flatten to column vectors (time-major order)
grid_t_full_col = reshape(grid_t_full, [], 1); 
grid_x_full_col = reshape(grid_x_full, [], 1);
grid_y_full_col = reshape(grid_y_full, [], 1);

xy_est = [grid_t_full_col, grid_x_full_col, grid_y_full_col]; 
clear grid_t_full;
...
coll_est_3D = reshape(coll_est, n_y, n_x, n_t); 
...
switch detrend_method
    ...
    poly_fit_full_3D = reshape(z_poly_est, n_y, n_x, n_t);
...
switch detrend_method
    ...
    final_signal = coll_est_3D;

    poly_fit_full_3D = zeros(size(final_signal));
    ...
    final_signal = coll_est_3D + poly_fit_full_3D;
...
coll_est_grid_shift = coll_est_3D - coll_est_3D(:,:,1);
poly_fit_full_grid_shift = poly_fit_full_3D - poly_fit_full_3D(:,:,1);
...
% t_obs, x_obs, y_obs are already defined in Step 4.1
...
% xy_est is [n_total x 3] -> [Time, X, Y]
...
switch detrend_method
    ...
    % Prediction grid
    A_2D_trend = [xy_est(:,2) - x0, xy_est(:,3) - y0];
    % Observation grid
    A_obs_trend = [xy_obs(:,2) - x0, xy_obs(:,3) - y0];
    ...
    if ~use_inclined_means
        ...
        % Large AOI: inclined means (3 params per epoch)
        n_atmos_params = 3 * n_epochs_LSC;
        
        % Prediction: atmosphere is NOT predicted (zeros)
        A_2D_atmos = zeros(n_2D_total, n_atmos_params);
        
        % Observations: block ramps (Bias, Sx, Sy)
        ...
        % Build sparse matrix
        I = repmat((1:n_curr_obs)', 3, 1);
        J = [col_bias; col_slopeX; col_slopeY];
        % Values: 1, (x-x0), (y-y0)
        V = [ones(n_curr_obs, 1); (xy_obs(:,2)-x0); (xy_obs(:,3)-y0)];
    ...
    if poly_degree >= 1
        A_poly_2D = [A_poly_2D, x_2D_n, y_2D_n, t_2D_n];
    end
    
    if poly_degree >= 2
        A_poly_2D = [A_poly_2D, x_2D_n.^2, y_2D_n.^2, t_2D_n.^2, ...
                      x_2D_n.*y_2D_n, x_2D_n.*t_2D_n, y_2D_n.*t_2D_n];
    end
    if poly_degree >= 3
        A_poly_2D = [A_poly_2D, x_2D_n.^3, y_2D_n.^3, t_2D_n.^3, ...
                      x_2D_n.^2.*y_2D_n, x_2D_n.^2.*t_2D_n, ...
                      y_2D_n.^2.*x_2D_n, y_2D_n.^2.*t_2D_n, ...
                      t_2D_n.^2.*x_2D_n, t_2D_n.^2.*y_2D_n, ...
                      x_2D_n.*y_2D_n.*t_2D_n];
    end
...
% Reshape for output
final_std = reshape(sqrt(var_2D), n_y, n_x, n_t);
final_std_noise_3D = reshape(sqrt(var_2D + A_noise), n_y, n_x, n_t);
...
lonlatIN_AOI_STC2D = [lon_full_shp, lat_full_shp];
...
% final_signal is [n_y x n_x x n_t], lonlatIN_AOI_STC2D is [n_x*n_y x 2]
% reshape final_signal to [n_spat x n_t], y-major order
final_signal_out = reshape(final_signal_shift, n_spat, n_t);  % [n_x*n_y x n_t]
final_std_out = reshape(final_std, n_spat, n_t);              % [n_x*n_y x n_t]

final_signal_out = final_signal_out(xyIN_AOI_flag, :);
final_std_out = final_std_out(xyIN_AOI_flag, :);

% verify sizes match
assert(size(final_signal_out, 1) == size(lonlatIN_AOI_STC2D, 1), ...
    'Mismatch: final_signal_out has %d rows, lonlatIN_AOI_STC2D has %d rows.', ...
    size(final_signal_out, 1), size(lonlatIN_AOI_STC2D, 1));
...
% OUTPUT:
% lonlatIN_AOI_STC2D = lonlatIN_AOI_STC2D;
...
%%           -------- END OF SCRIPT -------
disp('===============================================');
disp('--- 2D Stochastic-based modelling COMPLETED ---');
disp('===============================================');